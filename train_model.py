...
def create_data_loaders(train_dir, test_dir, eval_dir, train_batch_size, test_batch_size):
    ...
    """
    bucket_name = 'sagemaker-us-east-1-378124415251'
    parent_folder = 'data-project'
    base_path = 'dogImages'

    #     # Train data download
    #     s3_train_path = 's3://' + bucket_name + '/' + parent_folder + '/train'
    #     local_train = base_path + '/train'

    # #     if not os.listdir(local_train):
    # #         cp = CloudPath(s3_train_path)
    # #         cp.download_to(local_train)
    # #     else:
    # #         print('Train folder not empty')

    # #     # Test data download
    #     s3_test_path = 's3://' + bucket_name + '/' + parent_folder + '/test'
    #     local_test = base_path + '/test'

    # #     if not os.listdir(local_test):
    # #         cp = CloudPath(s3_test_path)
    # #         cp.download_to(local_test)
    # #     else:
    # #         print('Test folder not empty')

    # #     # Validation data download
    #     s3_valid_path = 's3://' + bucket_name + '/' + parent_folder + '/valid'
    #     local_valid = base_path + '/valid'

    # #     if not os.listdir(local_valid):
    # #         cp = CloudPath(s3_valid_path)
    # #         cp.download_to(local_valid)
    # #     else:
    # #         print('Validation folder not empty')
